...
$sitemap .= '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
    ...
    for($i=0;$i<=$month_diff;++$i){ $sm_date=date('Y-m',strtotime('-'.$i.' month',strtotime($curent_month))); //sitemap
        $sitemap_url='sitemap-posts-' .$sm_date.'.xml'; $sitemap .='<sitemap>' ; $sitemap .='<loc>'
        .url($sitemap_url).'</loc>';
        $sitemap .= '<lastmod>'.date('c',strtotime('-1 hours')).'</lastmod>';
        $sitemap .= '</sitemap>';
        }
...
$sitemap_category .='<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
    xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">
    ';
...
$sitemap .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
    xmlns:news="http://www.google.com/schemas/sitemap-news/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
$sitemap_posts .='<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">
    ';

    $posts = Story::where(['status'=>1])->whereYear('created_at', '=', $year)
    ->whereMonth('created_at', '=', $month)->get();

    foreach($posts as $post){
    $sitemap_posts .= '<url>';
        $sitemap_posts .= '<loc>'.getUrlPost($post).'</loc>';
        $sitemap_posts .= '<lastmod>'.date('c',strtotime($post->created_at)).'</lastmod>';
